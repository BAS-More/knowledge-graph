...
describe('loadSettings', () => {
  ...
  it('returns defaults on corrupted JSON', () => {
    sessionStorage.setItem('knowledge-graph-llm-settings', 'not-json{{{');
    const settings = loadSettings();
    expect(settings.activeProvider).toBeDefined();
  });
  ...
  it('migrates legacy localStorage to sessionStorage', () => {
    ...
    expect(sessionStorage.getItem('knowledge-graph-llm-settings')).not.toBeNull();
    expect(localStorage.getItem('knowledge-graph-llm-settings')).toBeNull();
  });
});
...
describe('saveSettings / clearSettings', () => {
  ...
  it('clearSettings removes settings from both storages', () => {
    ...
    expect(sessionStorage.getItem('knowledge-graph-llm-settings')).not.toBeNull();
    clearSettings();
    expect(sessionStorage.getItem('knowledge-graph-llm-settings')).toBeNull();
    expect(localStorage.getItem('knowledge-graph-llm-settings')).toBeNull();
  });
});
