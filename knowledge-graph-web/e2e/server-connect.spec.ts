...
/**
 * E2E tests for the Knowledge-Graph web UI — exploring view features.
 *
 * Requires:
 *   - knowledge-graph serve running on localhost:4747 with at least one indexed repo
 *   - knowledge-graph-web dev server running on localhost:5173
 *
...
test.beforeAll(async () => {
  ...
  try {
    ...
    if (
      ...
      test.skip(true, 'knowledge-graph serve not available on :4747');
      return;
    }
    if (
    ...
    if (backendRes.status === 'fulfilled') {
      ...
      if (!repos.length) {
        test.skip(true, 'No indexed repos — run knowledge-graph analyze first');
        return;
      }
    }
  } catch {
...
async function waitForGraphLoaded(page: import('@playwright/test').Page) {
  ...
  const preferredLandingCard = landingCards
    .filter({ hasText: /Knowledge-Graph|local-integration/ })
    .first();
  try {
