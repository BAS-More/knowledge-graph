...
- Renamed all internal paths from `kuzu` to `lbug` (storage: `.knowledge-graph/kuzu` → `.knowledge-graph/lbug`)
...
- **Optional skill generation** via `npx knowledge-graph analyze --skills` — generates AI agent skills from KuzuDB knowledge graph (#171) — @zander-raycraft
...
- Auto-reindex hook that runs `knowledge-graph analyze` after commits and merges, with automatic embeddings preservation (#205) — @L1nusB
...
- **Lazy CLI module loading**: All CLI subcommands now use `createLazyAction()` to defer heavy imports (tree-sitter, ONNX, KuzuDB) until invocation, significantly improving `knowledge-graph mcp` startup time (#207)
