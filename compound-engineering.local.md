...
## Project Overview
Knowledge-Graph is a code intelligence tool that builds a knowledge graph from source code using tree-sitter AST parsing across 12 languages and KuzuDB for graph storage. Two packages: `knowledge-graph/` (CLI/MCP, TypeScript) and `knowledge-graph-web/` (browser).

## Cross-Language Pattern Consistency (pattern-recognition-specialist)
- 12 language-specific type extractors in `knowledge-graph/src/core/ingestion/type-extractors/` must follow identical patterns for: async unwrapping, constructor binding, namespace handling, nullable type stripping, for-loop element typing.
...
- KuzuDB graph operations: schema in `knowledge-graph/src/core/kuzu/schema.ts`, adapter in `kuzu-adapter.ts`.
...
- `knowledge-graph-web` package drifts behind CLI — flag if a change should be mirrored.
...
**Why:** Knowledge-Graph is an MCP server — all tool inputs come from untrusted AI agents. Systematic OWASP-level audit catches injection vectors that spot-checking misses. Past finding: `lbug-adapter.ts` fallback path had unescaped newlines in Cypher queries.
...
## Review Tooling
- Use `knowledge-graph_impact()` before approving changes to any symbol — check d=1 (WILL BREAK) callers.
- Use `knowledge-graph_detect_changes({scope: "compare", base_ref: "main"})` to map PR diffs to affected execution flows.
