...
**Paths:** All file paths are relative to the monorepo root (`Knowledge-Graph/`). Git commands run from the root. The `knowledge-graph/` prefix is a package subdirectory, not a separate repo.
...
**Files:**
- Modify: `knowledge-graph/src/core/group/storage.ts:17-19` (getGroupDir) and `:63-68` (createGroupDir)
- Test: `knowledge-graph/test/unit/group/storage.test.ts`

- [ ] **Step 1: Write failing tests for validateGroupName**

In `knowledge-graph/test/unit/group/storage.test.ts`, add `createGroupDir` and `validateGroupName` to the existing import from `'../../../src/core/group/storage.js'` (line 6-11). Then add these describe blocks at the end of the outer `describe('Group storage', ...)`:
...
Run: `cd knowledge-graph && npx vitest run test/unit/group/storage.test.ts`
...
In `knowledge-graph/src/core/group/storage.ts`, add the validation function before `getGroupDir` and call it:
...
export function getGroupDir(knowledge-graphDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(knowledge-graphDir, 'groups', groupName);
}
```
...
```bash
cd knowledge-graph && git add src/core/group/storage.ts test/unit/group/storage.test.ts
...
**Files:**
- Modify: `knowledge-graph/src/core/group/service-boundary-detector.ts:24-51` (add constant), `:78` (walkForBoundaries), `:130` (hasSourceFilesInSubdirs)
- Test: `knowledge-graph/test/unit/group/service-boundary-detector.test.ts`

- [ ] **Step 1: Write failing tests for excluded directories**

Add this describe block inside the existing `detectServiceBoundaries` describe in `knowledge-graph/test/unit/group/service-boundary-detector.test.ts`:
...
Run: `cd knowledge-graph && npx vitest run test/unit/group/service-boundary-detector.test.ts`
...
In `knowledge-graph/src/core/group/service-boundary-detector.ts`:
...
```bash
cd knowledge-graph && git add src/core/group/service-boundary-detector.ts test/unit/group/service-boundary-detector.test.ts
...
**Files:**
- Modify: `knowledge-graph/src/cli/group.ts:160` (remove import), `:187-189` (remove finally block body)
- Test: `knowledge-graph/test/unit/group/sync.test.ts` (add pool cleanup test)
- Test: `knowledge-graph/test/integration/group/group-cli.test.ts` (verify no blanket close in source)

- [ ] **Step 1: Write unit tests for per-id pool cleanup in sync.ts**

Add to `knowledge-graph/test/unit/group/sync.test.ts`, inside the existing `describe('syncGroup', ...)`:
...
Run: `cd knowledge-graph && npx vitest run test/unit/group/sync.test.ts`
...
Add to `knowledge-graph/test/integration/group/group-cli.test.ts`:
...
Run: `cd knowledge-graph && npx vitest run test/integration/group/group-cli.test.ts`
Expected: FAIL — `closeLbug()` (no args) exists at line 188.

- [ ] **Step 5: Remove blanket closeLbug() from cli/group.ts**

In `knowledge-graph/src/cli/group.ts`:
...
Run: `cd knowledge-graph && npx vitest run test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts`
Expected: ALL PASS

- [ ] **Step 7: Commit**

```bash
cd knowledge-graph && git add src/cli/group.ts test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts
...
**Files:**
- Modify: `knowledge-graph/src/core/group/extractors/grpc-extractor.ts:101-130` (parseProtoFile)
- Test: `knowledge-graph/test/unit/group/grpc-extractor.test.ts`

- [ ] **Step 1: Write failing tests for nested braces in proto services**

Add this describe block inside the existing `proto file parsing` describe in `knowledge-graph/test/unit/group/grpc-extractor.test.ts`:
...
Run: `cd knowledge-graph && npx vitest run test/unit/group/grpc-extractor.test.ts`
...
In `knowledge-graph/src/core/group/extractors/grpc-extractor.ts`, replace the `parseProtoFile` method (lines 101-130):
...
```bash
cd knowledge-graph && git add src/core/group/extractors/grpc-extractor.ts test/unit/group/grpc-extractor.test.ts
...
Run: `cd knowledge-graph && npx vitest run test/unit/group/ test/integration/group/`
Expected: ALL PASS

- [ ] **Step 2: Run full test suite to catch regressions**

Run: `cd knowledge-graph && npx vitest run`
Expected: ALL PASS, 0 failures

- [ ] **Step 3: Run typecheck**

Run: `cd knowledge-graph && npx tsc --noEmit`
Expected: No errors
...
Add to `knowledge-graph/test/integration/group/group-cli.test.ts` inside the existing `group CLI` describe:
...
```bash
cd knowledge-graph && git add test/integration/group/group-cli.test.ts
