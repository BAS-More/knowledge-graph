...
All notable changes to Knowledge-Graph will be documented in this file.
...
- **Runtime dependency on `file:../knowledge-graph-shared`** removed from the published package (#803)
...
- **`KNOWLEDGE_GRAPH_HOME` env var** to customize global directory (#746)
...
- **`.knowledge-graphignore` negation patterns** now respected (#654)
- **MCP setup** prefers global knowledge-graph binary over npx (#653)
...
### Fixed
- **`knowledge-graph-shared` module not found** — `knowledge-graph-shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `knowledge-graph` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `knowledge-graph-shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `knowledge-graph-shared` package for cross-package type unification (#536)
...
- **`--skip-agents-md` CLI flag** — opt out of overwriting Knowledge-Graph-managed sections in AGENTS.md and CLAUDE.md during `knowledge-graph analyze` (#517)
...
### Docs
- Added `knowledge-graph-shared` build step to web UI quick start instructions (#585)
- Added enterprise offering section to README (#579)

## [1.4.10] - 2026-03-27

### Fixed
- **MCP server install via npx** — resolve tree-sitter peer dependency conflicts that broke `npx -y knowledge-graph@latest mcp` (#537, #538)
...
- `knowledge-graph-stable-ops` added to community integrations
...
- `.gitignore` and `.knowledge-graphignore` support during file discovery (#231)
