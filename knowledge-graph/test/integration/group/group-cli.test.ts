/**
 * Smoke-test `knowledge-graph group` CLI via tsx (same pattern as cli-e2e.test.ts).
...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'knowledge-graph-group-cli-'));
});
...
function runGroup(args: string[]) {
  return spawnSync(process.execPath, ['--import', tsxImportUrl, cliEntry, 'group', ...args], {
    ...
    env: { ...process.env, KNOWLEDGE_GRAPH_HOME: tmpHome },
  });
}
