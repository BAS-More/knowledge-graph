 ...
 * - analyzeCommand: verify pipeline runs and creates .knowledge-graph/ output
 *
...
afterAll(() => {
  // Clean up .git/ and .knowledge-graph/ directories created during the test
  for (const dir of ['.git', '.knowledge-graph']) {
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .knowledge-graph/ output directory
    const knowledgeGraphDir = path.join(MINI_REPO, '.knowledge-graph');
    expect(fs.existsSync(knowledgeGraphDir)).toBe(true);
    expect(fs.statSync(knowledgeGraphDir).isDirectory()).toBe(true);
  });
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/knowledge-graph/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // finds the parent project's .knowledge-graph. Use an isolated temp git
      // repo to guarantee no .knowledge-graph exists anywhere in the path.
  ...
  describe('wiki command flags', () => {
    ...
    it('wiki on non-indexed repo fails with "No Knowledge-Graph index"', () => {
      ...
      try {
        ...
        // Must spawn outside project tree so it doesn't find parent .knowledge-graph
        ...
        expect(result.status).toBe(1);
        expect(result.stdout).toMatch(/No Knowledge-Graph index found/);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  // Requires analyze to have run first (the analyze test above populates .knowledge-graph/).
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('KNOWLEDGE_GRAPH_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('KNOWLEDGE_GRAPH_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
