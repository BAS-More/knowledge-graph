 ...
 *
 * Tests the hook scripts with real git repos and .knowledge-graph directories.
...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'knowledge-graph-hook.cjs');
const PLUGIN_HOOK = path.resolve(
  __dirname,
  '..',
  '..',
  '..',
  'knowledge-graph-claude-plugin',
  'hooks',
  'knowledge-graph-hook.js',
);
...
// ─── Temp git repo with .knowledge-graph ───────────────────────────────────

let tmpDir: string;
let knowledgeGraphDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hooks-e2e-'));
  knowledgeGraphDir = path.join(tmpDir, '.knowledge-graph');
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      fs.writeFileSync(
        path.join(knowledgeGraphDir, 'meta.json'),
      ...
      expect(output!.additionalContext).toContain('npx knowledge-graph analyze');
    });
    ...
    it('treats missing meta.json as stale', () => {
      // Remove meta.json
      const metaPath = path.join(knowledgeGraphDir, 'meta.json');
  ...
  describe('PreToolUse — silent without knowledge-graph CLI', () => {
    // PreToolUse tries to spawn `knowledge-graph augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `knowledge-graph augment` hangs and the 10s timeout kills the process.
  ...
  describe('unhappy paths', () => {
    it('handles corrupted meta.json (invalid JSON) without crashing', () => {
      fs.writeFileSync(path.join(knowledgeGraphDir, 'meta.json'), 'THIS IS NOT JSON {{{');
    ...
    it('handles meta.json with missing lastCommit field', () => {
      fs.writeFileSync(path.join(knowledgeGraphDir, 'meta.json'), JSON.stringify({ stats: {} }));
  ...
  describe('directory without .knowledge-graph', () => {
    // The hook walks up 5 parent directories looking for .knowledge-graph.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .knowledge-graph could exist in any ancestor.
    let noKnowledgeGraphDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .knowledge-graph
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-knowledge-graph-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noKnowledgeGraphDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noKnowledgeGraphDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noKnowledgeGraphDir, stdio: 'pipe' });
    });
    ...
    afterAll(() => {
      ...
      const base = path.join(
        root,
        path.basename(path.resolve(noKnowledgeGraphDir, '..', '..', '..', '..', '..', '..')),
      );
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .knowledge-graph directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
        tool_output: { exit_code: 0 },
        cwd: noKnowledgeGraphDir,
      });
    ...
    it('ignores PreToolUse when no .knowledge-graph directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
