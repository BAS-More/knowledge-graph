...
describe('indexCommand', () => {
  ...
  beforeEach(() => {
    ...
    mockGetStoragePaths.mockImplementation((repoPath: string) => ({
      storagePath: `${repoPath}/.knowledge-graph`,
      lbugPath: `${repoPath}/.knowledge-graph/lbug`,
      metaPath: `${repoPath}/.knowledge-graph/meta.json`,
    }));
  ...
  it('fails when .knowledge-graph folder does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockRejectedValueOnce(new Error('missing .knowledge-graph'));
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `  No .knowledge-graph/ folder found at: ${resolvedRepo}/.knowledge-graph`,
    );
  });
  ...
  it('fails when lbug database does not exist', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(logSpy).toHaveBeenCalledWith(
      '  .knowledge-graph/ folder exists but contains no LadybugDB index.',
    );
  });
