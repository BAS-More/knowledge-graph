 ...
 *
 * Tests the hook scripts (knowledge-graph-hook.cjs and knowledge-graph-hook.js) that run
 * as PreToolUse and PostToolUse hooks in Claude Code.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Glob/Bash tool inputs
 * - findKnowledgeGraphDir: .knowledge-graph directory discovery
...
// ─── Test fixtures: temporary .knowledge-graph directory ───────────────────

let tmpDir: string;
let knowledgeGraphDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'knowledge-graph-hook-test-'));
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows knowledge-graph binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain('knowledge-graph.cmd');
  });
});
