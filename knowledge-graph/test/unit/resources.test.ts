...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find((d) => d.uri === 'knowledge-graph://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find((d) => d.uri === 'knowledge-graph://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes context, clusters, processes, schema, cluster detail, process detail', () => {
    ...
    expect(uris).toContain('knowledge-graph://repo/{name}/context');
    expect(uris).toContain('knowledge-graph://repo/{name}/clusters');
    expect(uris).toContain('knowledge-graph://repo/{name}/processes');
    expect(uris).toContain('knowledge-graph://repo/{name}/schema');
    expect(uris).toContain('knowledge-graph://repo/{name}/cluster/{clusterName}');
    expect(uris).toContain('knowledge-graph://repo/{name}/process/{processName}');
  });
...
describe('readResource', () => {
  it('routes knowledge-graph://repos to listRepos', async () => {
    ...
    const result = await readResource('knowledge-graph://repos', backend);
  ...
  it('routes knowledge-graph://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        {
          name: 'proj',
    ...
    const result = await readResource('knowledge-graph://setup', backend);
    expect(result).toContain('Knowledge-Graph MCP');
  ...
  it('routes knowledge-graph://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('knowledge-graph://repo/test-project/context', backend);
  ...
  it('routes knowledge-graph://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('knowledge-graph://repo/any/schema', backend);
    expect(result).toContain('Knowledge-Graph Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes knowledge-graph://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
        clusters: [{ heuristicLabel: 'Auth', symbolCount: 10, cohesion: 0.9 }],
      },
    });
    const result = await readResource('knowledge-graph://repo/test/clusters', backend);
  ...
  it('routes knowledge-graph://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [{ heuristicLabel: 'LoginFlow', processType: 'intra_community', stepCount: 3 }],
      },
    });
    const result = await readResource('knowledge-graph://repo/test/processes', backend);
  ...
  it('routes knowledge-graph://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('knowledge-graph://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('knowledge-graph://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes knowledge-graph://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('knowledge-graph://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('knowledge-graph://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('knowledge-graph://unknown', backend)).rejects.toThrow(
  ...
  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('knowledge-graph://repo/test/nonexistent', backend)).rejects.toThrow(
      'Unknown resource',
    );
  });
  ...
  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('knowledge-graph://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('knowledge-graph://repo/test/cluster/Auth%20Module', backend);
