...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.knowledge-graph');
  ...
  it('creates or updates CLAUDE.md with Knowledge-Graph section', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);

    const claudeMdPath = path.join(tmpDir, 'CLAUDE.md');
    const content = await fs.readFile(claudeMdPath, 'utf-8');
    expect(content).toContain('knowledge-graph:start');
    expect(content).toContain('knowledge-graph:end');
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    // Should only have one knowledge-graph section
    const starts = (content.match(/knowledge-graph:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'knowledge-graph');
    try {
