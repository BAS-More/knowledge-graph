...
// By default, execFileSync throws (simulating `which knowledge-graph` not found)
...
describe('setupClaudeCode', () => {
  ...
  it('writes win32 MCP entry with cmd wrapper', async () => {
    ...
    expect(config.mcpServers['knowledge-graph']).toEqual({
      command: 'cmd',
  ...
  it('preserves existing keys in ~/.claude.json', async () => {
    ...
    expect(config.mcpServers['knowledge-graph']).toBeDefined();
  });
  ...
  it('uses global binary path when knowledge-graph is on PATH', async () => {
    setPlatform('darwin');
    execFileSyncMock.mockReturnValueOnce('/usr/local/bin/knowledge-graph\n');
    ...
    expect(config.mcpServers['knowledge-graph']).toEqual({
      command: '/usr/local/bin/knowledge-graph',
      args: ['mcp'],
    });
  });

  it('falls back to npx when knowledge-graph is not on PATH', async () => {
    setPlatform('darwin');
