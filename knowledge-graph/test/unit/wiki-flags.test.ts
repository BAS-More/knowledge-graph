...
describe('resolveLLMConfig', () => {
  ...
  beforeEach(async () => {
    ...
    const configDir = path.join(tmpDir, '.knowledge-graph');
...
describe('--verbose flag', () => {
  const originalEnv = process.env.KNOWLEDGE_GRAPH_VERBOSE;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.KNOWLEDGE_GRAPH_VERBOSE;
    } else {
      process.env.KNOWLEDGE_GRAPH_VERBOSE = originalEnv;
    }
  });

  it('verboseLog writes to console when KNOWLEDGE_GRAPH_VERBOSE=1', async () => {
    ...
    expect(process.env.KNOWLEDGE_GRAPH_VERBOSE).toBe('1');

    consoleSpy.mockRestore();
  });

  it('verbose is off when KNOWLEDGE_GRAPH_VERBOSE is not set', () => {
    delete process.env.KNOWLEDGE_GRAPH_VERBOSE;
    expect(process.env.KNOWLEDGE_GRAPH_VERBOSE).toBeUndefined();
  });
});
