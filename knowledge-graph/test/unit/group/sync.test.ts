...
describe('syncGroup', () => {
  ...
  it('per-repo extractorOverride receives repo handle and extracts per repo', async () => {
    ...
    const result = await syncGroup(config, {
      ...
      resolveRepoHandle: async (_name, groupPath) => ({
        ...
        storagePath: '/tmp/' + groupPath + '/.knowledge-graph',
      }),
      skipWrite: true,
    });
  ...
  it('writes registry to groupDir when skipWrite is false', async () => {
    const tmpDir = path.join(os.tmpdir(), `knowledge-graph-sync-write-${Date.now()}`);
...
describe('stableRepoPoolId', () => {
  it('returns lowercase name when no collision', () => {
    const entry: RegistryEntry = {
      name: 'MyRepo',
      path: '/a/MyRepo',
      storagePath: '/a/MyRepo/.knowledge-graph',
      indexedAt: '',
      lastCommit: '',
    };
    const all = [entry];
  ...
  it('appends hash suffix on name collision with different path', () => {
    const entry1: RegistryEntry = {
      name: 'repo',
      path: '/a/repo',
      storagePath: '/a/repo/.knowledge-graph',
      indexedAt: '',
      lastCommit: '',
    };
    const entry2: RegistryEntry = {
      name: 'repo',
      path: '/b/repo',
      storagePath: '/b/repo/.knowledge-graph',
      indexedAt: '',
      lastCommit: '',
    };
