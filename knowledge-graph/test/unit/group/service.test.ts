...
function makeTmpGroup(): { tmpDir: string; groupDir: string; cleanup: () => void } {
  const tmpDir = path.join(os.tmpdir(), `knowledge-graph-svc-${Date.now()}`);
...
describe('GroupService', () => {
  describe('groupList', () => {
    it('test_groupList_without_name_returns_group_names', async () => {
      ...
      try {
        vi.stubEnv('KNOWLEDGE_GRAPH_HOME', tmpDir);
      ...
      try {
        ...
        const port = makePort({
          resolveRepo: vi.fn(async (name?: string) => {
            ...
            return { id: 'fe', name: 'fe', repoPath: '/tmp', storagePath: '/tmp/.knowledge-graph' };
          }),
