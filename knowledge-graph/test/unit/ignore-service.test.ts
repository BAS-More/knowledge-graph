...
describe('loadIgnoreRules', () => {
  ...
  it('parses .knowledge-graphignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.knowledge-graphignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.knowledge-graphignore'));
  });
...
describe('createIgnoreFilter', () => {
  ...
  it('childrenIgnored respects negation patterns (exclude-all + whitelist)', async () => {
    // Reproduces https://github.com/BAS-More/knowledge-graph/issues/596
    // Pattern: `*` (exclude all) + `!iOS/` + `!iOS/**` (whitelist iOS)
    await fs.writeFile(
      path.join(tmpDir, '.knowledge-graphignore'),
      '*\n!iOS/\n!iOS/**\n!backend/\n!backend/living_plan/\n!backend/living_plan/**\n',
    );
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('childrenIgnored respects negation patterns without trailing slash (!dir vs !dir/)', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.knowledge-graphignore'), '*\n!iOS\n!iOS/**\n');
    const filter = await createIgnoreFilter(tmpDir);
...
describe('loadIgnoreRules — KNOWLEDGE_GRAPH_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when KNOWLEDGE_GRAPH_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');

    const original = process.env.KNOWLEDGE_GRAPH_NO_GITIGNORE;
    process.env.KNOWLEDGE_GRAPH_NO_GITIGNORE = '1';
    try {
      const ig = await loadIgnoreRules(tmpDir);
      ...
      if (original === undefined) {
        delete process.env.KNOWLEDGE_GRAPH_NO_GITIGNORE;
      } else {
        process.env.KNOWLEDGE_GRAPH_NO_GITIGNORE = original;
      }
      await fs.unlink(path.join(tmpDir, '.gitignore'));
    }
  });

  it('still reads .knowledge-graphignore when KNOWLEDGE_GRAPH_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.knowledge-graphignore'), 'vendor/\n');
