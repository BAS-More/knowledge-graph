...
describe('getStoragePath', () => {
  it('appends .knowledge-graph to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.knowledge-graph');
    expect(path.basename(result)).toBe('.knowledge-graph');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, lbugPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.knowledge-graph');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.knowledge-graph/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('knowledge-graph-config-test-');
