 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .knowledge-graph/ index.
...
import { NODE_TABLES, type GraphNode, type GraphRelationship } from 'knowledge-graph-shared';
 ...
 * - https://knowledge-graph.vercel.app — the deployed Knowledge-Graph web UI
 *
...
export const isAllowedOrigin = (origin: string | undefined): boolean => {
  ...
  if (
    ...
    origin === 'http://[::1]' ||
    origin === 'https://knowledge-graph.vercel.app'
  ) {
    return true;
  }
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  const resolveRepo = async (repoName?: string, isRetry = false, req?: any): Promise<any> => {
    ...
    // e.g. "C:\Users\LENOVO\.knowledge-graph\repos\todo.txt-cli" -> "todo.txt-cli"
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: knowledge-graph analyze' });
        return;
      }
  ...
  app.delete('/api/repo', async (req, res) => {
    try {
      ...
      try {
        ...
        // 1. Delete the .knowledge-graph index/storage directory
        const storagePath = getStoragePath(entry.path);
        await fs.rm(storagePath, { recursive: true, force: true }).catch(() => {});

        // 2. Delete the cloned repo dir if it lives under ~/.knowledge-graph/repos/
  ...
  await new Promise<void>((resolve, reject) => {
    const server = app.listen(port, host, () => {
      ...
      console.log(`Knowledge-Graph server running on http://${displayHost}:${port}`);
      resolve();
    });
