 ...
 * Repository Manager
 *
 * Manages Knowledge-Graph index storage in .knowledge-graph/ at repo root.
 * Also maintains a global registry at ~/.knowledge-graph/registry.json
...
/**
 * Shape of an entry in the global registry (~/.knowledge-graph/registry.json)
 */
...
const KNOWLEDGE_GRAPH_DIR = '.knowledge-graph';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .knowledge-graph storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), KNOWLEDGE_GRAPH_DIR);
};
 ...
 *
 * Returns:
 *   found        — true if .knowledge-graph/kuzu existed and was deleted
...
/**
 * Check if a path has a Knowledge-Graph index
 */
...
/**
 * Find .knowledge-graph by walking up from a starting path
 */
...
/**
 * Add .knowledge-graph to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(KNOWLEDGE_GRAPH_DIR)) return;

    const newContent = content.endsWith('\n')
      ? `${content}${KNOWLEDGE_GRAPH_DIR}\n`
      : `${content}\n${KNOWLEDGE_GRAPH_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${KNOWLEDGE_GRAPH_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.knowledge-graph/registry.json) ───────────────────────

/**
 * Get the path to the global Knowledge-Graph directory
 */
export const getGlobalDir = (): string => {
  return process.env.KNOWLEDGE_GRAPH_HOME || path.join(os.homedir(), '.knowledge-graph');
};
 ...
 * Called after `knowledge-graph analyze` completes.
 */
 ...
 * Called after `knowledge-graph clean`.
 */
 ...
 * Optionally validates that each entry's .knowledge-graph/ still exists.
 */
export const listRegisteredRepos = async (opts?: {
  ...
  // Validate each entry still has a .knowledge-graph/ directory
...
// ─── Global CLI Config (~/.knowledge-graph/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.knowledge-graph/config.json
 */
...
/**
 * Save CLI config to ~/.knowledge-graph/config.json
 */
