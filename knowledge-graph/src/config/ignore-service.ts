...
// NOTE: Negation patterns in .knowledge-graphignore (e.g. `!vendor/`) cannot override
...
/**
 * Load .gitignore and .knowledge-graphignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .knowledge-graphignore. Defaults to KNOWLEDGE_GRAPH_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
 ...
 * - .gitignore / .knowledge-graphignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .knowledge-graphignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // Check against .gitignore / .knowledge-graphignore patterns.
