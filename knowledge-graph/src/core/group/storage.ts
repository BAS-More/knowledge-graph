...
export function getDefaultKnowledgeGraphDir(): string {
  return process.env.KNOWLEDGE_GRAPH_HOME || path.join(os.homedir(), '.knowledge-graph');
}

export function getGroupsBaseDir(knowledgeGraphDir?: string): string {
  return path.join(knowledgeGraphDir || getDefaultKnowledgeGraphDir(), 'groups');
}
...
export function getGroupDir(knowledgeGraphDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(knowledgeGraphDir, 'groups', groupName);
}
...
export async function listGroups(knowledgeGraphDir?: string): Promise<string[]> {
  const groupsDir = getGroupsBaseDir(knowledgeGraphDir);
  try {
...
export async function createGroupDir(
  ...
  const groupDir = getGroupDir(knowledgeGraphDir, groupName);
