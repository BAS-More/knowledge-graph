 ...
 * Retries on lock errors (e.g., when `knowledge-graph analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: knowledge-graph analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `knowledge-graph analyze` is writing.
