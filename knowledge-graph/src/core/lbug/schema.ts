...
import { NODE_TABLES, REL_TABLE_NAME, REL_TYPES, EMBEDDING_TABLE_NAME } from 'knowledge-graph-shared';
...
export type { NodeTableName, RelType } from 'knowledge-graph-shared';
...
const _rawDims = parseInt(process.env.KNOWLEDGE_GRAPH_EMBEDDING_DIMS ?? '384', 10);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  throw new Error(
    `KNOWLEDGE_GRAPH_EMBEDDING_DIMS must be a positive integer, got "${process.env.KNOWLEDGE_GRAPH_EMBEDDING_DIMS}"`,
  );
}
