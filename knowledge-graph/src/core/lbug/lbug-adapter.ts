 ...
 * an exclusive lock on the LadybugDB file (e.g. `knowledge-graph analyze` or
 * `knowledge-graph serve` running at the same time).
 */
 ...
 * database is busy (e.g. `knowledge-graph analyze` holds the write lock).
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        console.error('Knowledge-Graph: FTS extension load failed:', msg);
      }
    }
  }
};
...
export const loadVectorExtension = async (): Promise<void> => {
  ...
  try {
    ...
    if (
      ...
      console.error('Knowledge-Graph: VECTOR extension load failed:', msg);
    }
  }
};
/**
