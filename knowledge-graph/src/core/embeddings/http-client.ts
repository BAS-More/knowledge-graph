 ...
 * Returns null when KNOWLEDGE_GRAPH_EMBEDDING_URL + KNOWLEDGE_GRAPH_EMBEDDING_MODEL are unset.
 * Not cached — env vars are read fresh so late configuration takes effect.
 */
const readConfig = (): HttpConfig | null => {
  const baseUrl = process.env.KNOWLEDGE_GRAPH_EMBEDDING_URL;
  const model = process.env.KNOWLEDGE_GRAPH_EMBEDDING_MODEL;
  if (!baseUrl || !model) return null;

  const rawDims = process.env.KNOWLEDGE_GRAPH_EMBEDDING_DIMS;
  ...
  return {
    ...
    model,
    apiKey: process.env.KNOWLEDGE_GRAPH_EMBEDDING_API_KEY ?? 'unused',
    dimensions,
  };
};
    ...
    for (const item of items) {
      ...
      if (vec.length !== expected) {
        const hint = config.dimensions
          ? 'Update KNOWLEDGE_GRAPH_EMBEDDING_DIMS to match your model output.'
          : `Set KNOWLEDGE_GRAPH_EMBEDDING_DIMS=${vec.length} to match your model output.`;
        throw new Error(
...
export const httpEmbedQuery = async (text: string): Promise<number[]> => {
  ...
  if (embedding.length !== expected) {
    const hint = config.dimensions
      ? 'Update KNOWLEDGE_GRAPH_EMBEDDING_DIMS to match your model output.'
      : `Set KNOWLEDGE_GRAPH_EMBEDDING_DIMS=${embedding.length} to match your model output.`;
    throw new Error(
