 ...
 *
 * Provides tool implementations using local .knowledge-graph/ indexes.
...
// AI context generation is CLI-only (knowledge-graph analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`Knowledge-Graph [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(
          `Knowledge-Graph: "${entry.name}" has a stale KuzuDB index. Run: knowledge-graph analyze ${entry.path}`,
        );
      }
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: knowledge-graph analyze');
    }
    if (repoParam) {
  ...
  private async query(
    ...
    return {
      ...
      ...(!ftsUsed && {
        warning:
          'FTS extension unavailable - keyword search degraded. Run: knowledge-graph analyze --force to rebuild indexes.',
      }),
    };
  }
  ...
  private async impact(
    ...
    try {
      ...
      return {
        ...
        impactedCount: 0,
        risk: 'UNKNOWN',
        suggestion: 'The graph query failed — try knowledge-graph context <symbol> as a fallback',
      };
    }
  }
