 ...
 * All resources use repo-scoped URIs: knowledge-graph://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    ...
    {
      uri: 'knowledge-graph://setup',
      name: 'Knowledge-Graph Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'knowledge-graph://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'knowledge-graph://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'knowledge-graph://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'knowledge-graph://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'knowledge-graph://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'knowledge-graph://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'knowledge-graph://repos') return { resourceType: 'repos' };
  if (uri === 'knowledge-graph://setup') return { resourceType: 'setup' };

  // Repo-scoped: knowledge-graph://repo/{name}/context
  const repoMatch = uri.match(/^knowledge-graph:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: knowledge-graph analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# knowledge-graph_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: knowledge-graph analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx knowledge-graph analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - knowledge-graph://repos: All indexed repositories');
  lines.push(`  - knowledge-graph://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - knowledge-graph://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - knowledge-graph://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - knowledge-graph://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: knowledge-graph analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use knowledge-graph_query for deeper search.`,
      );
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: knowledge-graph analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use knowledge-graph_query for deeper search.`,
      );
    }
...
function getSchemaResource(): string {
  return `# Knowledge-Graph Graph Schema
 ...
 * Useful for `knowledge-graph setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# Knowledge-Graph\n\nNo repositories indexed. Run: `npx knowledge-graph analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# Knowledge-Graph MCP — ${repo.name}`,
      '',
      `This project is indexed by Knowledge-Graph as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`knowledge-graph://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`knowledge-graph://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`knowledge-graph://repo/${repo.name}/processes\` — All execution flows`,
      `- \`knowledge-graph://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
