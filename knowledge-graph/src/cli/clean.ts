 ...
 * Clean Command
 *
 * Removes the .knowledge-graph index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete Knowledge-Graph indexes for ${entries.length} repo(s):`);
  ...
  if (!options?.force) {
    console.log(`This will delete the Knowledge-Graph index for: ${repoName}`);
