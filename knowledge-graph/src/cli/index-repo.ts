 ...
 * Index Command
 *
 * Registers an existing .knowledge-graph/ folder into the global registry so the
 * MCP server can discover the repo without running a full `knowledge-graph analyze`.
 *
 * Useful when a pre-built .knowledge-graph/ directory is already present (e.g. after
...
export const indexCommand = async (inputPathParts?: string[], options?: IndexOptions) => {
  ...
  try {
    ...
    console.log(`  No .knowledge-graph/ folder found at: ${storagePath}`);
    console.log('  Run `knowledge-graph analyze` to build the index first.\n');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`  .knowledge-graph/ folder exists but contains no LadybugDB index.`);
    console.log('  Run `knowledge-graph analyze` to build the index.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (!meta) {
    if (!options?.force) {
      console.log(`  .knowledge-graph/ exists but meta.json is missing.`);
      console.log('  Use --force to register anyway (stats will be empty),');
      console.log('  or run `knowledge-graph analyze` to rebuild properly.\n');
      process.exitCode = 1;
      return;
    }
