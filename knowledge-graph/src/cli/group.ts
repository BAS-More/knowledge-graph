// knowledge-graph/src/cli/group.ts
...
export function registerGroupCommands(program: Command): void {
  ...
  group
    ...
    .action(async (groupName: string, groupPath: string, registryName: string) => {
      const { getGroupDir, getDefaultKnowledgeGraphDir } = await import('../core/group/storage.js');
      const { loadGroupConfig } = await import('../core/group/config-parser.js');
      const path = await import('node:path');
      const fs = await import('node:fs/promises');
      const groupDir = getGroupDir(getDefaultKnowledgeGraphDir(), groupName);
      ...
      console.log(`Run: knowledge-graph group sync ${groupName}`);
    });
  ...
  group
    ...
    .action(async (name?: string) => {
      ...
      if (!name) {
        ...
        if (groups.length === 0) {
          console.log('No groups configured. Create one with: knowledge-graph group create <name>');
          return;
        }
        console.log('Groups:');
      ...
      const { loadGroupConfig } = await import('../core/group/config-parser.js');
      const groupDir = getGroupDir(getDefaultKnowledgeGraphDir(), name);
      const config = await loadGroupConfig(groupDir);
  ...
  group
    ...
    .action(async (name: string) => {
      const { readContractRegistry, getGroupDir, getDefaultKnowledgeGraphDir } =
        await import('../core/group/storage.js');
