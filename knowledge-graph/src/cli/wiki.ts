 ...
 * Usage: knowledge-graph wiki [path] [options]
 */
...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  ...
  if (options?.verbose) {
    process.env.KNOWLEDGE_GRAPH_VERBOSE = '1';
  }

  console.log('\n  Knowledge-Graph Wiki Generator\n');
  ...
  if (!meta) {
    console.log('  Error: No Knowledge-Graph index found.');
    console.log('  Run `knowledge-graph analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.knowledge-graph/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or KNOWLEDGE_GRAPH_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        const envKey = process.env.KNOWLEDGE_GRAPH_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `knowledge-graph wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `knowledge-graph wiki` to continue.\n');
          return;
        }
      }
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `knowledge-graph wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
      if (process.env.KNOWLEDGE_GRAPH_VERBOSE) {
        console.error(err);
      }
    }
    process.exitCode = 1;
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki — generated by Knowledge-Graph', '--public'],
