 ...
 * so the Knowledge-Graph MCP server is available in all projects.
 */
...
/**
 * Resolve the absolute path to the `knowledge-graph` binary if it's installed
 * globally (or via npm -g / yarn global). Returns null when not found.
 */
function resolveGitnexusBin(): string | null {
  try {
    const cmd = process.platform === 'win32' ? 'where' : 'which';
    const resolved = execFileSync(cmd, ['knowledge-graph'], {
      encoding: 'utf-8',
      timeout: 5000,
 ...
 *
 * Prefers the globally-installed `knowledge-graph` binary (starts in ~1 s) over
 * `npx -y knowledge-graph@latest` (cold-cache install of native deps can take
 * >60 s, exceeding Claude Code's 30 s MCP connection timeout).
 *
 * Falls back to npx when the binary isn't on PATH — e.g. first-time
 * users who ran `npx knowledge-graph analyze` but haven't done `npm i -g`.
 */
function getMcpEntry() {
  ...
  if (process.platform === 'win32') {
    return {
      command: 'cmd',
      args: ['/c', 'npx', '-y', 'knowledge-graph@latest', 'mcp'],
    };
  }
  return {
    command: 'npx',
...
/**
 * Merge knowledge-graph entry into an existing MCP config JSON object.
...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers['knowledge-graph'] = getMcpEntry();
  return existing;
}
...
/**
 * Install Knowledge-Graph skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install Knowledge-Graph hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the knowledge-graph package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/knowledge-graph/
  const destHooksDir = path.join(claudeDir, 'hooks', 'knowledge-graph');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'knowledge-graph-hook.cjs');
    const dest = path.join(destHooksDir, 'knowledge-graph-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'knowledge-graph-hook.cjs').replace(/\\/g, '/');
    ...
    // Helper: add a hook entry if one with 'knowledge-graph-hook' isn't already registered
    interface HookEntry {
    ...
    function ensureHookEntry(
      ...
      const hasHook = existing.hooks[eventName].some((h: HookEntry) =>
        h.hooks?.some((hh) => hh.command?.includes('knowledge-graph-hook')),
      );
      if (!hasHook) {
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    config.mcp['knowledge-graph'] = getMcpEntry();
...
function getCodexMcpTomlSection(): string {
  ...
  return `[mcp_servers.knowledge-graph]\ncommand = ${command}\nargs = ${args}\n`;
}

/**
 * Append Knowledge-Graph MCP server config to Codex's config.toml if missing.
 */
async function upsertCodexConfigToml(configPath: string): Promise<void> {
  ...
  if (existing.includes('[mcp_servers.knowledge-graph]')) {
    return;
  }
...
/**
 * Install Knowledge-Graph skills to a target directory.
 * Each skill is installed as {targetDir}/knowledge-graph-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/knowledge-graph/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/knowledge-graph/
 */
...
/**
 * Install global Codex skills to ~/.agents/skills/knowledge-graph/
 */
...
export const setupCommand = async () => {
  console.log('');
  console.log('  Knowledge-Graph Setup');
  console.log('  ==============');
  console.log('');
  ...
  console.log('    2. Run: knowledge-graph analyze');
