 ...
 * Analyze Command
 *
 * Indexes a repository and stores the knowledge graph in .knowledge-graph/
 *
...
export interface AnalyzeOptions {
  ...
  skills?: boolean;
  verbose?: boolean;
  /** Skip AGENTS.md and CLAUDE.md knowledge-graph block updates. */
...
export const analyzeCommand = async (inputPath?: string, options?: AnalyzeOptions) => {
  ...
  if (process.env.KNOWLEDGE_GRAPH_NO_GITIGNORE) {
    console.log(
      '  KNOWLEDGE_GRAPH_NO_GITIGNORE is set — skipping .gitignore (still reading .knowledge-graphignore)\n',
    );
  }
  ...
  try {
    ...
    try {
      ...
      console.log('\n  Tip: Run `knowledge-graph setup` to configure MCP for your editor.');
    }
    ...
    if (
      ...
      console.error('    1. Add large vendored/generated directories to .knowledge-graphignore');
