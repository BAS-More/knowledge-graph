 ...
 *
 * Exposes Knowledge-Graph tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 *
 * Usage:
 *   knowledge-graph query "authentication flow"
 *   knowledge-graph context --name "validateUser"
 *   knowledge-graph impact --target "AuthService" --direction upstream
 *   knowledge-graph cypher "MATCH (n:Function) RETURN n.name LIMIT 10"
 *
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('Knowledge-Graph: No indexed repositories found. Run: knowledge-graph analyze');
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `knowledge-graph cypher ... | head -1`)
...
export async function queryCommand(
  ...
  if (!queryText?.trim()) {
    console.error('Usage: knowledge-graph query <search_query>');
    process.exit(1);
  }
...
export async function contextCommand(
  ...
  if (!name?.trim() && !options?.uid) {
    console.error('Usage: knowledge-graph context <symbol_name> [--uid <uid>] [--file <path>]');
    process.exit(1);
  }
...
export async function impactCommand(
  ...
  if (!target?.trim()) {
    console.error('Usage: knowledge-graph impact <symbol_name> [--direction upstream|downstream]');
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: 'Try reducing --depth or using knowledge-graph context <symbol> as a fallback',
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(
  ...
  if (!query?.trim()) {
    console.error('Usage: knowledge-graph cypher <cypher_query>');
    process.exit(1);
  }
