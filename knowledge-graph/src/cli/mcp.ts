...
export const mcpCommand = async () => {
  ...
  process.on('uncaughtException', (err) => {
    console.error(`Knowledge-Graph MCP: uncaught exception — ${err.message}`);
    // Process is in an undefined state after uncaughtException — exit after flushing
    setTimeout(() => process.exit(1), 100);
  });
  process.on('unhandledRejection', (reason) => {
    const msg = reason instanceof Error ? reason.message : String(reason);
    console.error(`Knowledge-Graph MCP: unhandled rejection — ${msg}`);
  });
  ...
  if (repos.length === 0) {
    console.error(
      'Knowledge-Graph: No indexed repos yet. Run `knowledge-graph analyze` in a git repo — the server will pick it up automatically.',
    );
  } else {
    console.error(
      `Knowledge-Graph: MCP server starting with ${repos.length} repo(s): ${repos.map((r) => r.name).join(', ')}`,
    );
  }
